...
library TruncatedOracle {
    ...
    /// @notice This is the max amount of liquidity in either direction that the pool is allowed to move at one time
    uint128 constant MAX_ABS_LIQUIDITY_MOVE = 10000; // should be adjusted based on irl testing
        ...
        returns (Observation memory)
    {
        unchecked {
            ...
            if (liquidity > last.prevLiquidity) {
                if ((liquidity - last.prevLiquidity) > MAX_ABS_LIQUIDITY_MOVE) {
                    liquidity = last.prevLiquidity + MAX_ABS_LIQUIDITY_MOVE;
                }
            } else {
                if ((last.prevLiquidity - liquidity) > MAX_ABS_LIQUIDITY_MOVE) {
                    liquidity = last.prevLiquidity - MAX_ABS_LIQUIDITY_MOVE;
                }
            }
